...
<template>
  ...
  >
    ...
    <p class="font-light text-gray-500 sm:text-lg dark:text-gray-400" :data-i18n="descriptionKey">
      &nbsp;
    </p>
    <div class="flex items-baseline justify-center my-8">
      <span class="mr-2 text-5xl font-extrabold">{{ price }}</span>
      <span class="text-gray-500 dark:text-gray-400" data-i18n="pricing_month">/month</span>
    </div>
    <ul role="list" class="mb-8 space-y-4 text-left">
      <li v-for="featureKey in featureKeys" :key="featureKey" class="flex items-center space-x-3">
        <svg
